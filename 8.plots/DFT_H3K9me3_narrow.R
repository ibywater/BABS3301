...
data <- read_tsv("tsv/DFT_H3K9me3_narrow_normalised_auto_data.tsv", skip=2, col_names=FALSE)
...
ggplot(plot_data, aes(x = position, y = signal, color = quartile)) +
  ...
  labs(x = "Position on Autosomes Relative to TSS (bp)",
       ...
       title = "DFT H3K9me3 (narrow) signal around TSS") +
  theme(plot.title = element_text(size = 18), axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14))

ggsave("actuals_plots/H3K9me3/DFT_H3K9me3_narrow_Quartiles_10kb_auto.png", dpi=300,
       width = 5.84, height = 3.95)

######### X Chromosome ##########

data <- read_tsv("tsv/DFT_H3K9me3_narrow_normalised_X_data.tsv", skip=2, col_names=FALSE)
...
ggsave("actuals_plots/H3K9me3/T1_DFT_H3K9me3_narrow_Quartiles_10kb_ChrX.png", dpi=300,
       width = 5.84, height = 3.95)
