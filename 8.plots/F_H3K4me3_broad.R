...
data <- read_tsv("tsv/F_H3K4me3_broad_normalised_auto_data.tsv", skip=2, col_names=FALSE)
...
plot_data <- plot_data %>%
  mutate(quartile = gsub("F_auto_", "", quartile)) %>%
...
ggplot(plot_data, aes(x = position, y = signal, color = quartile)) +
  ...
  labs(x = "Position on Autosomes Relative to TSS (bp)",
       ...
       title = "F_H3K4me3 (broad) signal around TSS") +
  theme(plot.title = element_text(size = 18), axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14))

ggsave("actuals_plots/H3K4me3/F_H3K4me3_broad_Quartiles_10kb_auto.png", dpi=300,
       width = 5.84, height = 3.95)

######### X Chromosome ##########

data <- read_tsv("tsv/F_H3K4me3_broad_normalised_X_data.tsv", skip=2, col_names=FALSE)
...
plot_data <- plot_data %>%
  mutate(quartile = gsub("F_X_", "", quartile)) %>%
  mutate(quartile = gsub(".bed", "", quartile))
...
ggplot(plot_data, aes(x = position, y = signal, color = quartile)) +
  ...
  labs(x = "Position on X chromosome Relative to TSS (bp)",
       y = "Normalised Read Depth",
       color = "Expression quartile",
       title = "DFT H3K4me3 (broad) signal around TSS") +
  theme(plot.title = element_text(size = 18), axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14))

ggsave("actuals_plots/H3K4me3/F_H3K4me3_broad_Quartiles_10kb_ChrX.png", dpi=300,
       width = 5.84, height = 3.95)
