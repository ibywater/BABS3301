...
data <- read_tsv("tsv/F_H3K9me3_broad_auto_data.tsv", skip=2, col_names=FALSE)
...
ggplot(plot_data, aes(x = position, y = signal, color = quartile)) +
  ...
  labs(x = "Position on Autosomes Relative to TSS (bp)",
       ...
       title = "Female H3K9me3 (broad) signal around TSS") +
  theme(plot.title = element_text(size = 18), axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14))

ggsave("actuals_plots/H3K9me3/F_H3K9me3_broad_Quartiles_10kb_auto.png", dpi=300,
       width = 5.84, height = 3.95)

######### X Chromosome ##########

data <- read_tsv("tsv/F_H3K9me3_broad_X_data.tsv", skip=2, col_names=FALSE)
...
ggsave("actuals_plots/H3K9me3/F_H3K9me3_broad_Quartiles_10kb_ChrX.png", dpi=300,
       width = 5.84, height = 3.95)
